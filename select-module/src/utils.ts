...
export function getElementAtPoint(x: number, y: number): HTMLElement {
  ...
  // - 不在选择器工具自身
  // - 确实在指定坐标内
  const refElement = (elementsStack.find(
    (element) =>
      !element.closest('svg') &&
...
export function getOffsetsFromPointToElement(
  ...
  const offsetTop =
    ((y - elementBounds.top) * 100) / elementBounds.height;
  const offsetLeft =
    ((x - elementBounds.left) * 100) / elementBounds.width;
...
export function getElementXPath(element: HTMLElement): string {
  ...
  for (; element && element.nodeType === Node.ELEMENT_NODE; element = element.parentNode as HTMLElement) {
    ...
    for (let sibling = element.previousSibling; sibling; sibling = sibling.previousSibling) {
      ...
      if ((sibling as HTMLElement).tagName === element.tagName) {
        index++;
      }
    }
